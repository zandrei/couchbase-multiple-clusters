...
public class SampleReactiveCouchbaseRepositoryFactory extends ReactiveRepositoryFactorySupport {
    /** Holds the reference to the template. */
    private final SampleReactiveRepositoryOperationsMapping couchbaseOperationsMapping;

    /** Holds the mapping context. */
    private final MappingContext<
                    ? extends CouchbasePersistentEntity<?>, CouchbasePersistentProperty>
            mappingContext;

    private final SampleCrudMethodMetadataPostProcessor crudMethodMetadataPostProcessor;
     ...
     */
    public SampleReactiveCouchbaseRepositoryFactory(
        ...
        this.crudMethodMetadataPostProcessor = new SampleCrudMethodMetadataPostProcessor();
    ...
    /**
     * Returns a new Repository based on the metadata. Two categories of repositories can be
     * instantiated: {@link SimpleReactiveCouchbaseRepository}. This method performs feature checks
     * to decide which of the two categories can be instantiated (eg. is N1QL available?).
     * Instantiation is done via reflection, see {@link
    ...
    /**
     * Returns the base class for the repository being constructed. Two categories of repositories
     * can be produced by this factory: {@link SimpleReactiveCouchbaseRepository} and. This method
     * checks if N1QL is available to choose between the two, but the actual concrete class is
     * determined respectively by. Override these methods if you want to change the base class for
    ...
    protected Optional<QueryLookupStrategy> getQueryLookupStrategy(
            QueryLookupStrategy.Key key, QueryMethodEvaluationContextProvider contextProvider) {
    ...
    private class CouchbaseQueryLookupStrategy implements QueryLookupStrategy {
        ...
        public CouchbaseQueryLookupStrategy(
        ...
        public RepositoryQuery resolveQuery(
            ...
            final ReactiveCouchbaseOperations couchbaseOperations =
                    couchbaseOperationsMapping.resolve(
                            metadata.getRepositoryInterface(), metadata.getDomainType());
            ReactiveCouchbaseQueryMethod queryMethod =
                    new ReactiveCouchbaseQueryMethod(method, metadata, factory, mappingContext);

            if (queryMethod.hasN1qlAnnotation()) {
                return new ReactiveStringBasedCouchbaseQuery(
                        queryMethod,
                        couchbaseOperations,
